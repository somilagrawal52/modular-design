...
const inquiryTypes = ['Space Capsule', 'Hotel or Retreat', 'Private Project', 'Commercial Space', 'Workplace', 'Community Amenity'];
            ...
            <Reveal direction="right" delay={0.4}>
              <div className="bg-stone/5 p-8 md:p-16 border border-stone/10 relative backdrop-blur-sm">
                ...
                <form className="space-y-12">
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] uppercase tracking-[0.4em] text-gold font-bold">What are you exploring?</label>
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] uppercase tracking-[0.4em] text-gold font-bold">Your brief</label>
                    <textarea
          ...
          <div className="lg:col-span-5 hidden lg:block">
            <ParallaxElement speed={-0.05} className="h-full">
              <div className="relative h-full overflow-hidden border border-stone/10 p-4 bg-stone/5">
                <ParallaxImage
                  src="/images/capsule-riverbend-resort-hero.png"
                  alt="Premium capsule accommodation in a landscape setting"
                  className="w-full h-full"
                />
                <div className="absolute inset-0 bg-ink/45" />
                <div className="absolute bottom-12 left-12 right-12">
                  <span className="text-[10px] uppercase tracking-[0.45em] text-gold font-bold">Choose the right starting point</span>
                  <p className="mt-5 text-2xl font-display font-medium leading-tight text-ivory">A model, an application, or a project question.</p>
                </div>
              </div>
            </ParallaxElement>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 pb-32">
          <Reveal direction="right">
            <div className="space-y-12">
              <h2 className="text-xs uppercase tracking-[0.5em] text-gold font-bold">Direct access</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {[
                  { label: 'Model enquiries', email: dreamRealtyEmail('models') },
                  { label: 'Project discussions', email: dreamRealtyEmail('projects') },
                  { label: 'General information', email: dreamRealtyEmail('studio') },
                  { label: 'Partnerships', email: dreamRealtyEmail('partners') },
                ].map((item) => (
                  <div key={item.label} className="group">
                    <h3 className="text-[10px] uppercase tracking-widest text-ivory/40 mb-2">{item.label}</h3>
          ...
          <Reveal direction="left">
            <div className="bg-gold/5 p-12 border border-gold/10">
              ...
              <h2 className="text-2xl font-display font-bold mb-6 tracking-tight">Model-led conversations</h2>
              <p className="text-ivory/75 font-light leading-relaxed mb-8">
                Start with a model or application. From there, the relevant configuration, finish, delivery, and project information can be considered against your requirements.
              </p>
              <a href={`mailto:${dreamRealtyEmail('studio')}`} className="inline-flex items-center gap-3 text-[10px] uppercase tracking-[0.3em] font-bold text-gold hover:text-ivory transition-colors">
